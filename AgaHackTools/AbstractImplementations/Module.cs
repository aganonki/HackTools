...
namespace AgaHackTools.Main.AbstractImplementations
{
    ...
    public abstract class Module<T> : IModule<T>
    {
        ...
        protected Module(T updateData,ILog logger = null, int ticks = 60)
        {
            ...
            Loop = new Task(() => Process(_state),TaskCreationOptions.LongRunning);
        ...
        public int FrameRate => lastFrameRate;

        public object Settings { get; set; }

        private Stopwatch _fps = new Stopwatch();
        private void CalculateFramerate()
        {
            if (!_fps.IsRunning)
                _fps.Start();
            else
            if (_fps.Elapsed.TotalMilliseconds >= 1000)
            {
                _fps.Reset();
