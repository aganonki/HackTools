...
{
    public class Misc: Module<CSGOData>
    { 
        #region Fields
        ...
        public Misc(ISmartMemory memory, CSGOData csgo, int tick = 100, ILog logger = null) : base(csgo, logger, tick)
        {
            _memory = memory;
            offsetsLoaded = false;
            OnUpdate += Module_Update;
        }
        public Misc(ISmartMemory memory, CSGOData csgo, int tick = 100) : base(csgo, Log.GetLogger(typeof(Misc).Name), tick)
        {
            _memory = memory;
            offsetsLoaded = false;
            OnUpdate += Module_Update;
        ...
        private void AutoPistol(CSGOData csgo)
        {
            if (csgo.Config.AutoPistol)
        ...
        public void bhop(CSGOData csgo)
        {
